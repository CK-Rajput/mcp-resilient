    ...
    **kwargs: Any,
    ...
    def _wrap_function(
        ...
        breaker = CircuitBreaker(
            config.tool_name, config.circuit_breaker, BreakerStateStore(backend)
        )
                ...
                for idx, tool in enumerate(chain):
                    ...
                    try:
                        if config.retry.enabled:
                            outcome = await run_with_retry(attempt, config.retry, tool)
    ...
    resolved_config: ReliabilityConfig

    if config_or_fn is None:
        ...
        resolved_config = ReliabilityConfig(tool_name=tool_name, **kwargs)

    elif not isinstance(config_or_fn, ReliabilityConfig):
        raise TypeError(
            "First argument to mcp_reliable() must be a ReliabilityConfig object, "
            "a decorated function, or keyword arguments. "
            f"Received: {type(config_or_fn).__name__}"
        )
    else:
        if kwargs:
            raise TypeError(
        ...
        resolved_config = config_or_fn

    def decorator(fn: Callable[..., Awaitable[T]]) -> Callable[..., Awaitable[T]]:
        if resolved_config.tool_name == "default_tool":
            resolved_config.tool_name = fn.__name__
        return _wrap_function(resolved_config, fn)
